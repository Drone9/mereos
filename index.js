	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (logonResp.data) {
				...
				resolve(logonResp.data);
			} else {
				reject(new Error('Failed to log in to the school'));
			}
		} catch (error) {
...
async function start_prechecks(setting) {
	return new Promise((resolve, reject) => {
		try {
			localStorage.setItem('precheckSetting', setting);
					
			const modalCallback = (result) => {
				if (result?.message === 'precheck_completed') {
					resolve(result);
				} else {
					reject(new Error('Precheck failed or incomplete'));
				}
			};

			window.globalCallback = modalCallback;
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (secureFeatures?.entities?.length > 0) {
				...
				window.startRecordingCallBack = (result) => {
					if (result?.message === 'recording_started_successfully') {
						resolve(result);
					} else {
						reject({ type: 'error', message: result?.message || 'Recording failed' });
					}
				};
		...
		try {
			...
			if (stopSessionResp === 'stop_recording') {
				...
				if (resp) {
					...
					resolve({ type: 'success', message: 'Session is finished successfully' });
				} else {
					throw new Error('Session can\'t be added');
				}
			} else {
				reject({ type: 'error', message: 'Stopping the recording failed' });
			}
		} catch (err) {
			logger.error(err);
			reject({ type: 'error', message: 'There was an error in stopping the session', details: err });
		}
	});
}
